...
# A reducer is a function that determines changes to an application's state. It uses the action it receives to determine this change. Redux help manage an application's state changes in a single store so that they behave consistently.

- [ x ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

# Application state is the "global" state that all components in the component tree can access, through cursors. This is the state that your application is in and basically what is being rendered. Component local state is state that is local to a single component and cannot be seen outside of this component.
